...
async (conn, mek, m, { from, q, reply, l }) => {
    try {
        ...
        let txt = `*🗃️ NATSU GDRIVE DOWNLOADER 🗃️*
...
async (conn, mek, m, {
  ...
  try {
    ...
    let caption = `🎶 *NAKSU SONG DOWNLOADER* 🎶
...
let caption = `*\`🪺 NAKSU TIK TOK DOWNLODER 🪺\`*
...
cmd({
    ...
    try {
        ...
        if (!thumb || !thumb.startsWith('http')) {
            thumb = 'https://files.catbox.moe/qfwspq.jpg';
        } else {
        ...
        const caption = `\`🏮 NAKSU FB DOWNLOADER 🏮\`\n\n` +
                   `*┌──────────────────*\n` +
...
async(conn, mek, m,{from, q, reply}) => {
  try {
    ...
    let listdata = `*\`🗃️ NAKSU APK DOWNLOADER 🗃️\`*
...
let listdata = `*\`🔥 🅽🅰🆃🆂🆄 𝙈𝙀𝘿𝙄𝘼𝙁𝙄𝙍𝙀 𝘿𝙊𝙒𝙉𝙇𝙊𝘿𝙀𝙍 🔥\`*
...
async (conn, mek, m, { from, prefix, q, reply }) => {
    try {
        ...
        const caption = `*\`🎼 NAKSU SOUNDCLOUD DOWNLOADER 🎼\`*\n\n*🎶 Title:* ${data.title}\n*🔗 URL:* ${q}`;
        ...
        const buttonMessage = {
            ...
            caption: caption,
            footer: config.FOOTER || 'NAKSU BOT',
            buttons: buttons,
            headerType: 4
        };
